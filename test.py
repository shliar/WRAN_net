...
def rransduction_module(datas,k,kappa,alpha):
    n, m, z = datas.shape    
    F1 = torch.zeros(n, m, m) #建立一个和datas一样的空矩阵
    ...
    for i in range(n):
        metric = distance_metrics()['cosine'] #计算每一维的余弦距离
        S = 1 - metric(ndatas_qr[i, :, :], ndatas_qr[i, :, :])
        S = torch.tensor(S)
        S = S - torch.eye(S.shape[0]) #生成一个余弦距离矩阵的对角矩阵
        if k>0:
...
if __name__ == '__main__':
    ...
    for kn in [4,6,8,10]:
        with open(result_file,'a') as f:
            f.write('k:{}\n'.format(kn))
        for kappa_n in [1]:   #[1,2,3,4]:
            with open(result_file,'a') as f:
                f.write('kappa:{}\n'.format(kappa_n))
            for bn in [0.2,0.5,0.7,0.9]:
                with open(result_file,'a') as f:
                    f.write('beta:{}:'.format(bn))
                n_queries = 15
                n_runs = 10000
                ...
                labels = torch.arange(n_ways).view(1, 1, n_ways).expand(n_runs, n_shot+n_queries, 3).clone().view(n_runs, n_samples)
                beta = bn
                ndatas[:, ] = torch.pow(ndatas[:, ]+1e-6, beta)
                st=time.time()
                ndatas = rransduction_module(ndatas, k=k, kappa=kappa, alpha=alpha)
